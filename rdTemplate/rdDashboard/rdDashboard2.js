...
YUI.add('dashboard-freeform', function (Y) {
    ...
    FreeForm.initializePanel = function (panelNode, resizerInstance) {
        ...
        var ngpVisualization = panelNode.one('.rdLogiVisualization logi-visualization');
        if (ngpVisualization) {
    ...
    FreeForm.resizeVisualizationToFitPanel = function (panel, chart) {
        ...
        if (availableWidth <= 5)
            availableWidth = 5;
        if (chart.get('tagName') === 'LOGI-VISUALIZATION') {
            var visConfigVarName = chart.getAttribute('config');
            var visConfig = window[visConfigVarName];
            if (visConfig && visConfig.style) {
                visConfig.style.width = availableWidth + "px";
                visConfig.style.height = availableHeight + "px";
            }
            ...
            var visualizationObject = Logi.Platform.select('#' + chart.getAttribute('id'));
            if (!visualizationObject) {
                return;
            }
            visualizationObject.resize(availableWidth + "px", availableHeight + "px");
        }
        //Get rid of the scroll bars
        panelBody.setStyle('overflow', 'hidden');
    };
    ...
    FreeForm.onPanelResizeEnd = function (e, panel, resizeChart, resizer) {
        ...
        if (panelNode.getData("oneChart")) {
            ...
            var ngpVisualization = panelNode.one('.logi-visualization-container');
            if (ngpVisualization) {
    ...
    FreeForm.panelFitToChart = function () {
        ...
        panels.each(function (panel) {
            ...
            if (charts.size() !== 1) {
                ...
                var ngpVisualization = panel.one('.logi-visualization-container');
                if (ngpVisualization) {
