...
Y.use('rdResize', function(Y) {	
	...
	LogiXML.Resize._rdInitVisualizationResizer = function (eleContent) {
	    ...
	    var resizeVisualization = function (width, height, finished) {
	        var visualizationDiv = resizeNode.one('.logi-visualization-container');
	        ...
	        rdAjaxRequest('rdAjaxCommand=rdAjaxNotify&rdNotifyCommand=SetElementSize&rdWidth=' + width + '&rdHeight=' + height + '&rdElementId=' + sIdForAttrs + '&rdReport=' + sReportName);
	    }
	    yuiResize.on('resize:resize', function (e) {
	        if (timeout) {
	            clearTimeout(timeout);
